...
class BeardGLTilemap  {
	...
	public static function renderThroughCamera (tilemap:BeardTileMap, renderSession:RenderSession, camera:Camera):Void {
		...
		if (!tilemap.__renderable || tilemap.__tiles.length == 0 || tilemap.__worldAlpha <= 0 || tilemap.tileset == null) return;
		...
		var visual, alpha, visible, tileData, tileMatrix, x, y, x2, y2, x3, y3, x4, y4;
		...
		for (i in 0...count) {
			...
			if (!visible || alpha <= 0) {
				...
				__skipTile (visual, i, offset, bufferData);
				continue;
				
			}
			
			//tileset = (visual.tileset != null) ? visual.tileset : defaultTileset;
			...
			tileData = tileset.__data[visual.id];
			...
			if (visual.id <= 0 || !camera.ContainsVisual(cast(visual, BeardVisual)))
			{
				__skipTile (visual, i, offset, bufferData);
				continue;
			}
			...
			if (alphaDirty || visual.__alphaDirty) {
				
				__updateTileAlpha (visual, worldAlpha, offset, bufferData);
				
			}
			
			if (visual.__sourceDirty) {
				
				__updateTileUV (visual, tileset, offset, bufferData);
				
			}
			
			if (camera.needRenderUpdate || visual.__transformDirty) {
				
				tileMatrix.setTo (1, 0, 0, 1, -visual.originX, -visual.originY);
				_adjustedMatrix.a = visual.matrix.a * camera.zoom;
				_adjustedMatrix.b = visual.matrix.b * camera.transform.d;
				_adjustedMatrix.c = visual.matrix.c * camera.zoom;
				_adjustedMatrix.d = visual.matrix.d * camera.transform.d;
				_adjustedMatrix.tx = camera.transform.tx + camera.viewportWidth*0.5 +  (visual.matrix.tx - camera.centerX) *camera.zoom;
				_adjustedMatrix.ty = camera.transform.ty + camera.viewportHeight *0.5 + (visual.matrix.ty - camera.centerY) *camera.zoom;
				
				
				tileMatrix.concat (_adjustedMatrix);
				
				x = visual.__transform[0] = tileMatrix.__transformX (0, 0);
				y = visual.__transform[1] = tileMatrix.__transformY (0, 0);
				x2 = visual.__transform[2] = tileMatrix.__transformX (tileWidth, 0);
				y2 = visual.__transform[3] = tileMatrix.__transformY (tileWidth, 0);
				x3 = visual.__transform[4] = tileMatrix.__transformX (0, tileHeight);
				y3 = visual.__transform[5] = tileMatrix.__transformY (0, tileHeight);
				x4 = visual.__transform[6] = tileMatrix.__transformX (tileWidth, tileHeight);
				y4 = visual.__transform[7] = tileMatrix.__transformY (tileWidth, tileHeight);
				
				visual.__transformDirty = false;
				
			} else {
				
				x = visual.__transform[0];
				y = visual.__transform[1];
				x2 = visual.__transform[2];
				y2 = visual.__transform[3];
				x3 = visual.__transform[4];
				y3 = visual.__transform[5];
				x4 = visual.__transform[6];
				y4 = visual.__transform[7];
