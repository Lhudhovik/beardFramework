...
class InputManagerPrev
{
	...
	private var handledInputs:Map<String, InputPrev>;
	...
	private var utilInput:InputPrev;
	private var mouseMoveTargetName:String;
	private var mouseTargetName:String;
	private var touchTargets:Map<String, String>;
	private var currentInput:InputPrev;
	private var defaultActions:Map<InputType, Signal1<InputPrev>>;
	public var focusedObject:String = "";
	private var triggeredInputs:MinAllocArray<InputPrev>;
	...
	private function Init():Void
	{
		...
		handledInputs = new Map<String, InputPrev>();
		...
		triggeredInputs = new MinAllocArray<InputPrev>(25);
	...
	public function Activate(window:Window):Void
	{
		...
		for (type in Type.allEnums(InputType))
			LinkActionToInput(GetAnyInputActionID(type), StringLibrary.ANY, type);
	...
	public function LinkActionToInput(actionID : String, inputID:String, inputType:InputType, ?compatibleActionsIDs : Array<String> = null):Void 
	{
		...
		if (handledInputs[inputID] == null){
			var input:InputPrev = {ID:inputID, state:InputType.NONE, value:0, target:"", toggle:GetToggleType(inputType), active:true};
	...
	public function Update():Void
	{
		...
		//for (i in 0...triggeredInputs.length)
		{
					...
					if (actions[actionID] != null){
						if (actions[actionID].active)
						{
							...
							while (--i >= 0)
							{
								...
								if (detail.target == currentInput.target || detail.target == "")
								{
			...
			if (inputActions[StringLibrary.ANY] != null && inputActions[StringLibrary.ANY][currentInput.state] != null)
			{
				for (actionID in inputActions[StringLibrary.ANY][currentInput.state])
				{
	...
	private function DirectResolveInput(input:InputPrev):Void
	{
		//trace("direct");
		if (input.active){
			currentInput = input;
					...
					if (actions[actionID] != null){
						if (actions[actionID].active)
						{
							...
							while (--i >= 0)
							{
								...
								if (detail.target == input.target || detail.target == "")
								{
	...
	public static inline function GetCallingInput():InputPrev
	{
	...
	public static inline function GetAnyInputActionID(inputType:InputType):String
	{
		
		return InputTypeToString(inputType) + StringLibrary.ANY;
